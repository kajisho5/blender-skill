...
| Script | Does |
|---|---|
| `info.py FILE` | Object/mesh/material/texture/animation/armature counts, unit scale, bounding box, non-manifold-edge and duplicate-vertex detection, approximate self-intersection detection, flipped-normal detection (skipped, not guessed, when non-manifold edges make it unreliable), unapplied-scale detection, origin offset from bbox center/bottom-center (data, not a defect), UV presence. The foundation every other script leans on. |
